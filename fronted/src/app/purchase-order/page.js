...
export default function PurchaseOrderForm({ onCancel }) {
  ...
  const updateItem = (index, field, value) => {
    ...
    updatedItems[index].tax_amount =
      (updatedItems[index].untaxed_amount * tax_rate) / 100;
    updatedItems[index].total_price =
      updatedItems[index].untaxed_amount + updatedItems[index].tax_amount;

    setItems(updatedItems);
  };

  const calculateTotals = () => {
    const totalUntaxed = items.reduce(
      (sum, item) => sum + item.untaxed_amount,
      0
    );
  ...
  const handleCreatePurchaseOrder = async (purchaseOrderData) => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000/api/purchase-orders",
        {
          method: "POST",
          headers: {
